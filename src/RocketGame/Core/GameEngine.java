...
public class GameEngine {
    ...
    private float findEnemyPosition(float width, float height, boolean isShooter) {
        ...
        float minSpacing = isShooter ? 120 : 60; // Shooters need more space
        int maxAttempts = isShooter ? 40 : 20;   // Try harder for shooter positions
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            for (Enemy existing : enemies) {
                ...
                if (isShooter && existing.getType() == Enemy.EnemyType.SHOOTER) {
                    // Special rule: Shooters avoid other shooters
                    if (distance < minSpacing) {
                        validPosition = false;
                        break;
                    }
                } else if (verticalDistance < 100) {
                    // General rule: Enemies shouldn't overlap when close vertically
        ...
        // Last resort: Try minimal spacing
